...
def vectorize_and_upload(df: pd.DataFrame, index_name: str):
    ...
    # Get existing IDs in the index
    existing_ids = set()
    # Pinecone does not provide a direct way to list all IDs, so we fetch by batch (if small dataset)
    # For large datasets, consider maintaining your own ID registry
    try:
        fetch_result = index.fetch(ids=[str(row['id']) for _, row in df.iterrows()])
        existing_ids = set(fetch_result['vectors'].keys())
    except Exception as e:
        print(f"Warning: Could not fetch existing IDs: {e}")
    # Prepare vectors, skipping existing IDs
    vectors = []
    for _, row in df.iterrows():
        row_id = str(row['id'])
        if row_id in existing_ids:
            print(f"Skipping existing ID: {row_id}")
            continue
        ...
        vectors.append((row_id, embedding, metadata))
    if vectors:
        index.upsert(vectors)
        print(f"Upserted {len(vectors)} new vectors to index '{index_name}'")
    else:
        print("No new vectors to upsert.")

# Create a mock Q&A CSV for the researcher agent
...
if __name__ == "__main__":
    csv_path = "./data/researcher_qa_mock.csv"
    create_mock_qa_csv(csv_path)
    df = load_dataset(csv_path)
    print_sample_rows(df)
    # Create Pinecone index for OpenAI embeddings if not exists
    create_pinecone_index("aiweekendv2", dimension=1536, metric="cosine")
    # Upsert to Pinecone index
    vectorize_and_upload(df, "aiweekendv2")
    # Test agent retrieval
    print("\n--- Agent Retrieval Test ---")
    query_agent("What are the main risks identified?", "aiweekendv2")
